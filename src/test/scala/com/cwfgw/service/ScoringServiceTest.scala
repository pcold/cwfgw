...
class ScoringServiceTest extends FunSuite:
  ...
  private val tid = UUID.fromString(
  ...
  private def makeResult(
    ...
    TournamentResult(
      UUID.randomUUID(), tid, golferId, position,
      Some(-5), Some(270), None, None,
      position.isDefined, Json.obj()
    )
  ...
  test("calculateGolferPayout: 1st place solo, 100% ownership") {
    ...
    val result = service.calculateGolferPayout(
      Some(1), results, BigDecimal(100), x1, rules
    )
  ...
  test("calculateGolferPayout: 1st place, 75% ownership") {
    ...
    val results = List(makeResult(gid, Some(1)))
    val result = service.calculateGolferPayout(
      Some(1), results, BigDecimal(75), x1, rules
    )
    val (base, owner, _) = result.get
    assertEquals(base, BigDecimal(18))
  ...
  test("calculateGolferPayout: 2x multiplier doubles payout") {
    ...
    val results = List(makeResult(gid, Some(1)))
    val result = service.calculateGolferPayout(
      Some(1), results, BigDecimal(100), x2, rules
    )
    val (base, owner, _) = result.get
  ...
  test("calculateGolferPayout: 1.5x multiplier") {
    ...
    val results = List(makeResult(gid, Some(1)))
    val result = service.calculateGolferPayout(
      Some(1), results, BigDecimal(100),
      BigDecimal("1.5"), rules
    )
    val (base, owner, _) = result.get
  ...
  test("calculateGolferPayout: T3 with 2 tied") {
    ...
    val results = List(
      makeResult(gid1, Some(3)),
      makeResult(gid2, Some(3))
    )
    val result = service.calculateGolferPayout(
      Some(3), results, BigDecimal(100), x1, rules
    )
    val (base, owner, _) = result.get
  ...
  test("calculateGolferPayout: position 11 returns None") {
    val result = service.calculateGolferPayout(
      Some(11), Nil, BigDecimal(100), x1, rules
    )
  ...
  test("calculateGolferPayout: no position returns None") {
    val result = service.calculateGolferPayout(
      None, Nil, BigDecimal(100), x1, rules
    )
    assertEquals(result, None)
  }
  ...
  test("calculateGolferPayout: breakdown JSON has correct fields") {
    ...
    val result = service.calculateGolferPayout(
      Some(5), results, BigDecimal(50), x2, rules
    )
    val (_, _, breakdown) = result.get
    assertEquals(
      breakdown.hcursor.downField("position").as[Int],
      Right(5)
    )
    assertEquals(
      breakdown.hcursor.downField("num_tied").as[Int],
      Right(1)
    )
    assertEquals(
      breakdown.hcursor.downField("base_payout")
        .as[BigDecimal],
      Right(BigDecimal(14))
    )
    assertEquals(
      breakdown.hcursor.downField("ownership_pct")
        .as[BigDecimal],
      Right(BigDecimal(50))
    )
    assertEquals(
      breakdown.hcursor.downField("payout")
        .as[BigDecimal],
      Right(BigDecimal(7))
    )
    assertEquals(
      breakdown.hcursor.downField("multiplier")
        .as[BigDecimal],
      Right(x2)
    )
  }
  ...
  test("calculateGolferPayout: 50% ownership of T10 at 2x with floor") {
    ...
    val results = List(
      makeResult(gid1, Some(10)),
      makeResult(gid2, Some(10)),
      makeResult(gid3, Some(10))
    )
    val result = service.calculateGolferPayout(
      Some(10), results, BigDecimal(50), x2, rules
    )
    val (base, owner, _) = result.get
  ...
  test("calculateGolferPayout: custom rules with fewer places") {
    ...
    val results = List(makeResult(gid, Some(1)))
    val result = service.calculateGolferPayout(
      Some(1), results, BigDecimal(100), x1, custom
    )
    assertEquals(result.get._1, BigDecimal(20))

    // Position 4 is outside custom payouts
    val none = service.calculateGolferPayout(
      Some(4), results, BigDecimal(100), x1, custom
    )
  ...
  test("zeroSumWeekly: single team always gets 0") {
    val result = service.zeroSumWeekly(
      List((teamId(1), BigDecimal(50)))
    )
    assertEquals(result.head._2, BigDecimal(0))
  }

  test("zeroSumWeekly: 13 teams sums to zero") {
    val teams = (1 to 13).toList
      .map(i => (teamId(i), BigDecimal(i * 3)))
  ...
  test("sideBetPnl: clear winner gets +$15*(N-1)") {
    val earnings = Map(
      teamId(1) -> BigDecimal(50),
      teamId(2) -> BigDecimal(30),
      teamId(3) -> BigDecimal(10)
    )
  ...
  test("sideBetPnl: all tied -> all get zero") {
    val earnings = Map(
      teamId(1) -> BigDecimal(20),
      teamId(2) -> BigDecimal(20)
    )
    val result = service.sideBetPnl(earnings)
  ...
  test("sideBetPnl: custom bet amount") {
    ...
    val result = service.sideBetPnl(
      earnings, sideBetAmount = BigDecimal(20)
    )
  ...
  test("sideBetPnl: zero-sum with 13 teams") {
    val earnings = (1 to 13)
      .map(i => teamId(i) -> BigDecimal(i)).toMap
    val result = service.sideBetPnl(earnings)
    assertEquals(result(teamId(13)), BigDecimal(180))
    (1 to 12).foreach(i =>
      assertEquals(result(teamId(i)), BigDecimal(-15))
    )
    assertEquals(result.values.sum, BigDecimal(0))
  }
