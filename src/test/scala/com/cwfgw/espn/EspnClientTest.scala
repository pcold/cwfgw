...
class EspnClientTest extends FunSuite:
  ...
  private def competitorJson(
    ...
    Json.obj(
      ...
      "status" -> Json.obj(
        "type" -> Json.obj("id" -> Json.fromString(statusId))
      )
    )
    ...
    Json.obj(
      "events" -> Json.arr(
        Json.obj(
          ...
          "status" -> Json.obj(
            "type" -> Json.obj("completed" -> Json.fromBoolean(completed))
          ),
          "competitions" -> Json.arr(
            Json.obj("competitors" -> Json.arr(competitors: _*))
          )
        )
      )
    )
  ...
  test("parseLeaderboard parses competitor fields") {
    val json = tournamentJson(competitors = List(
      competitorJson("42", "Scottie Scheffler", 1, "-15", roundScores = List(65, 67, 68, 66))
    ))
  ...
  test("tied scores share position") {
    ...
    assertEquals(competitors.map(c => (c.name, c.position)), List(
      ("Player A", 1),
      ("Player B", 2),
      ("Player C", 2),
  ...
  test("active player with 4 rounds is parsed with madeCut=true") {
    val json = tournamentJson(competitors = List(
      competitorJson("1", "Active Player", 1, "-5", statusId = "1", roundScores = List(68, 69, 70, 68))
    ))
  ...
  test("cut player (status 2) is parsed") {
    val json = tournamentJson(competitors = List(
      competitorJson("1", "Cut Player", 1, "+5", statusId = "2", roundScores = List(75, 76))
    ))
    val c = client.parseLeaderboard(json).toOption.get.competitors.head
  ...
  test("withdrawn player (status 3) is parsed") {
    val json = tournamentJson(competitors = List(
      competitorJson("1", "WD Player", 1, "+2", statusId = "3", roundScores = List(72))
    ))
    val c = client.parseLeaderboard(json).toOption.get.competitors.head
  ...
  test("disqualified player (status 4) is parsed") {
    val json = tournamentJson(competitors = List(
      competitorJson("1", "DQ Player", 1, "+1", statusId = "4", roundScores = List(71, 72))
    ))
    val c = client.parseLeaderboard(json).toOption.get.competitors.head
    ...
    val json = Json.obj(
      "events" -> Json.arr(
        Json.obj(
          ...
          "competitions" -> Json.arr(Json.obj("competitors" -> Json.arr(
            competitorJson("1", "Player A", 1, "-5")
          )))
        ),
        Json.obj(
  ...
  test("even par 'E' is parsed as scoreToPar=0") {
    val json = tournamentJson(competitors = List(
      competitorJson("1", "Even Player", 1, "E")
    ))
    val c = client.parseLeaderboard(json).toOption.get.competitors.head
