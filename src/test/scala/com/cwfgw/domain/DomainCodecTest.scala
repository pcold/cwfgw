...
/** Tests that domain model Circe codecs correctly round-trip through JSON with snake_case field naming as configured.
  */
class DomainCodecTest extends FunSuite:
  ...
  test("CreateSeason decodes from snake_case JSON") {
    val json = s"""{"league_id":"$sampleId","name":"S1","season_year":2026,"max_teams":10}"""
  ...
  test("CreateTournament decodes with minimal fields") {
    val json = s"""{"name":"Test Open","season_id":"$sampleId","start_date":"2026-01-15","end_date":"2026-01-18"}"""
  ...
  test("RosterEntry round-trips with all fields") {
    val entry = RosterEntry(sampleId, sampleId, sampleId, "draft", Some(1), BigDecimal(75), sampleInstant, None, true)
  ...
  test("AddToRoster decodes with optional fields") {
    val json = s"""{"golfer_id":"$sampleId","draft_round":3,"ownership_pct":50}"""
  ...
  test("SeasonStanding round-trips through JSON") {
    val standing = SeasonStanding(sampleId, sampleId, sampleId, BigDecimal("125.50"), 8, sampleInstant)
  ...
  test("SeasonStanding encodes with snake_case") {
    val standing = SeasonStanding(sampleId, sampleId, sampleId, BigDecimal(100), 5, sampleInstant)
