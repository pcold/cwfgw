...
class DomainCodecTest extends FunSuite:
  ...
  test("League encodes to snake_case JSON") {
    val league = League(sampleId, "Test League", 2026, "active", Json.obj(), 13, sampleInstant, sampleInstant)
  ...
  test("CreateLeague decodes from snake_case JSON") {
    val json = """{"name":"New League","season_year":2026,"max_teams":10}"""
    val result = decode[CreateLeague](json)
    assert(result.isRight)
    val cl = result.toOption.get
    assertEquals(cl.name, "New League")
    assertEquals(cl.seasonYear, 2026)
    assertEquals(cl.maxTeams, Some(10))
    assertEquals(cl.rules, None)
  }

  test("UpdateLeague decodes partial fields") {
    val json = """{"name":"Updated","status":"completed"}"""
    val result = decode[UpdateLeague](json)
    assert(result.isRight)
    val ul = result.toOption.get
    assertEquals(ul.name, Some("Updated"))
    assertEquals(ul.status, Some("completed"))
    assertEquals(ul.rules, None)
    assertEquals(ul.maxTeams, None)
  }
  ...
  test("Golfer encodes with snake_case fields") {
    val golfer = Golfer(sampleId, Some("12345"), "Scottie", "Scheffler", Some("USA"), Some(1), true, Json.obj(), sampleInstant)
  ...
  test("Tournament encodes with snake_case fields") {
    val t = Tournament(sampleId, Some("401"), "The Masters", 2026,
      LocalDate.of(2026, 4, 9), LocalDate.of(2026, 4, 12),
      Some("Augusta National"), "completed", Some(20000000L), true, Json.obj(), sampleInstant)
    val json = t.asJson
  ...
  test("CreateTournament decodes with minimal fields") {
    val json = """{"name":"Test Open","season_year":2026,"start_date":"2026-01-15","end_date":"2026-01-18","made_cut":true}"""
    val result = decode[CreateTournament](json)
    assert(result.isRight)
    val ct = result.toOption.get
    assertEquals(ct.name, "Test Open")
    assertEquals(ct.seasonYear, 2026)
    assertEquals(ct.pgaTournamentId, None)
    assertEquals(ct.isMajor, None)
  }

  test("TournamentResult round-trips") {
    val tr = TournamentResult(sampleId, sampleId, sampleId, Some(1), Some(-10), Some(270), Some(3600000L),
  ...
  test("Team encodes with snake_case fields") {
    val team = Team(sampleId, sampleId, "Alice", "Team A", Some(1), sampleInstant, sampleInstant)
  ...
  test("RosterEntry round-trips with all fields") {
    val entry = RosterEntry(sampleId, sampleId, sampleId, "draft", Some(1),
  ...
  test("AddToRoster decodes with optional fields") {
    val json = """{"golfer_id":"11111111-1111-1111-1111-111111111111","draft_round":3,"ownership_pct":50}"""
  ...
  test("Draft encodes with snake_case") {
    ...
    assert(json.hcursor.downField("started_at").focus.exists(_.isNull))
    assert(json.hcursor.downField("completed_at").focus.exists(_.isNull))
    assert(json.hcursor.downField("created_at").as[String].isRight)
  }

  test("DraftPick round-trips") {
    val pick = DraftPick(sampleId, sampleId, sampleId, Some(sampleId), 1, 1, Some(sampleInstant))
  ...
  test("FantasyScore round-trips through JSON") {
    val score = FantasyScore(sampleId, sampleId, sampleId, sampleId, sampleId,
      BigDecimal("18.50"), Json.obj("position" -> Json.fromInt(1)), sampleInstant)
  ...
  test("FantasyScore encodes with snake_case") {
    ...
    assert(json.hcursor.downField("tournament_id").as[String].isRight)
    assert(json.hcursor.downField("golfer_id").as[String].isRight)
    assert(json.hcursor.downField("calculated_at").as[String].isRight)
  }

  test("LeagueStanding round-trips through JSON") {
    val standing = LeagueStanding(sampleId, sampleId, sampleId, BigDecimal("125.50"), 8, sampleInstant)
    val decoded = decode[LeagueStanding](standing.asJson.noSpaces)
    assertEquals(decoded, Right(standing))
  }

  test("LeagueStanding encodes with snake_case") {
    val standing = LeagueStanding(sampleId, sampleId, sampleId, BigDecimal(100), 5, sampleInstant)
    val json = standing.asJson
    assert(json.hcursor.downField("total_points").as[BigDecimal].isRight)
    assert(json.hcursor.downField("tournaments_played").as[Int].isRight)
    assert(json.hcursor.downField("last_updated").as[String].isRight)
  }
  ...
  test("Golfer with null optional fields encodes nulls") {
    ...
    assert(json.hcursor.downField("world_ranking").focus.exists(_.isNull))
  }

  test("Tournament with zero purse amount") {
    val t = Tournament(sampleId, None, "Charity Open", 2026,
  ...
  test("BigDecimal precision preserved in FantasyScore") {
    ...
    val decoded = decode[FantasyScore](score.asJson.noSpaces)
    assertEquals(decoded.toOption.get.points, BigDecimal("7.333333333"))
  }
