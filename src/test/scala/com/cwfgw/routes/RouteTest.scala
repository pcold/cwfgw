...
class RouteTest extends FunSuite:
  ...
  private val sampleLeague =
    League(sampleId, "Test League", sampleInstant)
  ...
  private val leagueService = new LeagueService(null):
    override def list: IO[List[League]] =
  ...
  test("GET /api/v1/leagues/:id returns 200 when found") {
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(s"/api/v1/leagues/$sampleId")
    )
    val response = leagueRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.Ok)
    val body = response.as[Json].unsafeRunSync()
  ...
  test("GET /api/v1/leagues/:id returns 404 when not found") {
    ...
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(s"/api/v1/leagues/$otherId")
    )
    val response = leagueRoutes.run(req).unsafeRunSync()
  ...
  private val seasonRoutes =
    SeasonRoutes.routes(seasonService).orNotFound
  ...
  test("GET /api/v1/seasons with year param returns 200") {
    val req =
      Request[IO](Method.GET, uri"/api/v1/seasons?year=2026")
    val response = seasonRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.Ok)
  }
  ...
  test("GET /api/v1/seasons/:id returns 200 when found") {
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(s"/api/v1/seasons/$sampleId")
    )
    val response = seasonRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.Ok)
    val body = response.as[Json].unsafeRunSync()
  ...
  test("GET /api/v1/seasons/:id returns 404 when not found") {
    ...
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(s"/api/v1/seasons/$otherId")
    )
    val response = seasonRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.NotFound)
  }
  ...
  test("GET /api/v1/seasons/:id/standings returns 200") {
    val req = Request[IO](
      ...
      Uri.unsafeFromString(
        s"/api/v1/seasons/$sampleId/standings"
      )
    )
    val response = seasonRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.Ok)
  ...
  private val teamService = new TeamService(null):
    override def listBySeason(
        seasonId: UUID
    ): IO[List[Team]] =
      IO.pure(List(sampleTeam))
    override def get(id: UUID): IO[Option[Team]] =
      if id == sampleId then IO.pure(Some(sampleTeam))
      else IO.pure(None)
    override def getRoster(
        teamId: UUID
    ): IO[List[RosterEntry]] =
      IO.pure(Nil)
    override def dropFromRoster(
        teamId: UUID,
        golferId: UUID
    ): IO[Boolean] =
      IO.pure(false)
    override def getRosterView(
        seasonId: UUID
    ): IO[List[RosterViewTeam]] =
      IO.pure(Nil)

  private val teamRoutes =
    TeamRoutes.routes(teamService).orNotFound

  test("GET /api/v1/seasons/:id/teams returns 200") {
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(
        s"/api/v1/seasons/$sampleId/teams"
      )
    )
  ...
  test("GET /api/v1/seasons/:id/teams/:teamId returns 200") {
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(
        s"/api/v1/seasons/$sampleId/teams/$sampleId"
      )
    )
    val response = teamRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.Ok)
  ...
  test("GET /api/v1/seasons/:id/teams/:teamId returns 404") {
    ...
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(
        s"/api/v1/seasons/$sampleId/teams/$otherId"
      )
    )
    val response = teamRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.NotFound)
  ...
  test("GET /api/v1/seasons/:id/teams/:id/roster returns 200") {
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(
        s"/api/v1/seasons/$sampleId/teams/$sampleId/roster"
      )
    )
    val response = teamRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.Ok)
  ...
  test("DELETE roster entry returns 404 when not found") {
    ...
    val req = Request[IO](
      Method.DELETE,
      Uri.unsafeFromString(
        s"/api/v1/seasons/$sampleId/teams/$sampleId/roster/$otherId"
      )
    )
    val response = teamRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.NotFound)
  ...
  test("GET /api/v1/seasons/:id/rosters returns 200") {
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(
        s"/api/v1/seasons/$sampleId/rosters"
      )
    )
    val response = teamRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.Ok)
  ...
  private val scoringService = new ScoringService(null):
    override def getScores(
      ...
      IO.pure(Nil)
    override def calculateScores(
        seasonId: UUID,
        tournamentId: UUID
    ): IO[Either[String, Json]] =
      if seasonId == sampleId then
        IO.pure(Right(Json.obj("ok" -> true.asJson)))
      else IO.pure(Left("Season not found"))
    override def getSideBetStandings(
        seasonId: UUID
    ): IO[Either[String, Json]] =
      IO.pure(Right(Json.obj("rounds" -> Json.arr())))
    override def refreshStandings(
        seasonId: UUID
    ): IO[List[SeasonStanding]] =
      IO.pure(Nil)

  private val scoringRoutes =
    ScoringRoutes.routes(scoringService).orNotFound
  ...
  test("GET scoring returns 200") {
    ...
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(
        s"/api/v1/seasons/$sampleId/scoring/$tid"
      )
    )
  ...
  test("POST calculate scores returns 200 on success") {
    ...
    val req = Request[IO](
      Method.POST,
      Uri.unsafeFromString(
        s"/api/v1/seasons/$sampleId/scoring/calculate/$tid"
      )
    )
    val response = scoringRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.Ok)
  ...
  test("POST calculate scores returns 400 on error") {
    ...
    val req = Request[IO](
      Method.POST,
      Uri.unsafeFromString(
        s"/api/v1/seasons/$otherId/scoring/calculate/$tid"
      )
    )
    val response = scoringRoutes.run(req).unsafeRunSync()
  ...
  test("GET side-bets returns 200") {
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(
        s"/api/v1/seasons/$sampleId/scoring/side-bets"
      )
    )
    val response = scoringRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.Ok)
  ...
  test("POST refresh-standings returns 200") {
    val req = Request[IO](
      Method.POST,
      Uri.unsafeFromString(
        s"/api/v1/seasons/$sampleId/scoring/refresh-standings"
      )
    )
    val response = scoringRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.Ok)
  ...
  private val draftService = new DraftService(null):
    ...
    override def start(
        seasonId: UUID
    ): IO[Either[String, Draft]] =
      IO.pure(Left("Draft is already in_progress"))
    override def getPicks(
        seasonId: UUID
    ): IO[Either[String, List[DraftPick]]] =
      if seasonId == sampleId then IO.pure(Right(Nil))
      else IO.pure(Left("No draft found for this season"))
    override def getAvailableGolfers(
        seasonId: UUID
    ): IO[Either[String, List[Golfer]]] =
      IO.pure(Right(Nil))
    override def initializePicks(
        seasonId: UUID,
        rounds: Int
    ): IO[Either[String, List[DraftPick]]] =
      IO.pure(Left(
        "Draft picks can only be initialized when draft is pending"
      ))

  private val draftRoutes =
    DraftRoutes.routes(draftService).orNotFound

  test("GET draft returns 200 when found") {
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(
        s"/api/v1/seasons/$sampleId/draft"
      )
    )
  ...
  test("GET draft returns 404 when not found") {
    ...
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(
        s"/api/v1/seasons/$otherId/draft"
      )
    )
    val response = draftRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.NotFound)
  ...
  test("POST draft/start returns 400 on failure") {
    val req = Request[IO](
      Method.POST,
      Uri.unsafeFromString(
        s"/api/v1/seasons/$sampleId/draft/start"
      )
    )
    val response = draftRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.BadRequest)
    val body = response.as[Json].unsafeRunSync()
    assertEquals(
      body.hcursor.downField("error").as[String],
      Right("Draft is already in_progress")
    )
  }
  ...
  test("GET draft/picks returns 200 on success") {
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(
        s"/api/v1/seasons/$sampleId/draft/picks"
      )
    )
    val response = draftRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.Ok)
  ...
  test("GET draft/picks returns 400 when no draft exists") {
    ...
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(
        s"/api/v1/seasons/$otherId/draft/picks"
      )
    )
    val response = draftRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.BadRequest)
  ...
  test("GET draft/available returns 200") {
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(
        s"/api/v1/seasons/$sampleId/draft/available"
      )
    )
    val response = draftRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.Ok)
  ...
  test("POST draft/initialize returns 400 with error") {
    val req = Request[IO](
      Method.POST,
      Uri.unsafeFromString(
        s"/api/v1/seasons/$sampleId/draft/initialize"
      )
    )
    val response = draftRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.BadRequest)
  ...
  private val reportService =
    new WeeklyReportService(null, null):
      ...
      override def getGolferHistory(
        ...
        IO.pure(Json.obj(
          "golfer_name" -> "Scottie Scheffler".asJson
        ))

  private val reportRoutes =
    ReportRoutes.routes(reportService).orNotFound
  ...
  test("GET report returns 200") {
    ...
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(
        s"/api/v1/seasons/$sampleId/report/$tid"
      )
    )
  ...
  test("GET report with live param returns 200") {
    ...
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(
        s"/api/v1/seasons/$sampleId/report/$tid?live=true"
      )
    )
    val response = reportRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.Ok)
  ...
  test("GET rankings returns 200") {
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(
        s"/api/v1/seasons/$sampleId/rankings"
      )
    )
    val response = reportRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.Ok)
  ...
  test("GET golfer history returns 200") {
    ...
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(
        s"/api/v1/seasons/$sampleId/golfer/$golferId/history"
      )
    )
    val response = reportRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.Ok)
    val body = response.as[Json].unsafeRunSync()
    assertEquals(
      body.hcursor.downField("golfer_name").as[String],
  ...
  private val tournamentService =
    new TournamentService(null, null, null):
      override def list(
          seasonId: Option[UUID],
          status: Option[String]
      ): IO[List[Tournament]] =
        val filtered =
          if seasonId.contains(sampleId)
          then List(sampleTournament)
          else Nil
        IO.pure(filtered)
      override def get(id: UUID): IO[Option[Tournament]] =
        if id == sampleId then IO.pure(Some(sampleTournament))
        else IO.pure(None)
      override def getResults(
          tournamentId: UUID
      ): IO[List[TournamentResult]] =
        IO.pure(Nil)
      override def importResults(
          tournamentId: UUID,
          results: List[CreateTournamentResult]
      ): IO[List[TournamentResult]] =
        IO.pure(Nil)
      override def finalizeTournament(
          tournamentId: UUID
      ): IO[Either[String, String]] =
        if tournamentId == sampleId then
          IO.pure(Right("Tournament finalized"))
        else IO.pure(Left("Tournament not found"))
      override def resetTournament(
          tournamentId: UUID
      ): IO[Either[String, String]] =
        if tournamentId == sampleId then
          IO.pure(Right("Tournament reset"))
        else IO.pure(Left("Tournament not found"))
      override def finalizeSeason(
          seasonId: UUID
      ): IO[Either[String, String]] =
        if seasonId == sampleId then
  ...
  private val tournamentAdminRoutes =
    TournamentRoutes.adminRoutes(tournamentService).orNotFound

  test("GET /api/v1/tournaments returns 200") {
    val req =
      Request[IO](Method.GET, uri"/api/v1/tournaments")
  ...
  test("GET /api/v1/tournaments?season_id filters by season") {
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(
        s"/api/v1/tournaments?season_id=$sampleId"
      )
    )
    val response = tournamentRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.Ok)
  ...
  test("GET /api/v1/tournaments?season_id returns empty for unknown") {
    ...
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(
        s"/api/v1/tournaments?season_id=$otherId"
      )
    )
    val response = tournamentRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.Ok)
  ...
  test("GET /api/v1/tournaments/:id returns 200 when found") {
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(s"/api/v1/tournaments/$sampleId")
    )
    val response = tournamentRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.Ok)
    val body = response.as[Json].unsafeRunSync()
  ...
  test("GET /api/v1/tournaments/:id returns 404 when not found") {
    ...
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(s"/api/v1/tournaments/$otherId")
    )
    val response = tournamentRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.NotFound)
  }
  ...
  test("GET /api/v1/tournaments/:id/results returns 200") {
    val req = Request[IO](
      Method.GET,
      Uri.unsafeFromString(
        s"/api/v1/tournaments/$sampleId/results"
      )
    )
    val response = tournamentRoutes.run(req).unsafeRunSync()
    assertEquals(response.status, Status.Ok)
  ...
  test("POST finalize tournament returns 200 on success") {
    val req = Request[IO](
      Method.POST,
      Uri.unsafeFromString(
        s"/api/v1/tournaments/$sampleId/finalize"
      )
    )
    val response =
  ...
  test("POST finalize tournament returns 400 on failure") {
    ...
    val req = Request[IO](
      Method.POST,
      Uri.unsafeFromString(
        s"/api/v1/tournaments/$otherId/finalize"
      )
    )
    val response =
      tournamentAdminRoutes.run(req).unsafeRunSync()
  ...
  test("POST reset tournament returns 200 on success") {
    val req = Request[IO](
      Method.POST,
      Uri.unsafeFromString(
        s"/api/v1/tournaments/$sampleId/reset"
      )
    )
    val response =
      tournamentAdminRoutes.run(req).unsafeRunSync()
  ...
  test("POST reset tournament returns 400 on failure") {
    ...
    val req = Request[IO](
      Method.POST,
      Uri.unsafeFromString(
        s"/api/v1/tournaments/$otherId/reset"
      )
    )
    val response =
      tournamentAdminRoutes.run(req).unsafeRunSync()
  ...
  test("POST finalize season returns 200 on success") {
    val req = Request[IO](
      Method.POST,
      Uri.unsafeFromString(
        s"/api/v1/seasons/$sampleId/finalize"
      )
    )
    val response =
      tournamentAdminRoutes.run(req).unsafeRunSync()
