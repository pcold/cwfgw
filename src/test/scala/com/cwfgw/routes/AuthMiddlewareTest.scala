...
class AuthMiddlewareTest extends FunSuite:
  ...
  private def mkAuthService: AuthService =
    ...
    new AuthService(null, sessions):
      override def login(username: String, password: String): IO[Option[String]] = IO.pure(None)

  /** A simple protected route that returns 200 with a body. */
  private val innerRoutes: HttpRoutes[IO] = HttpRoutes.of[IO]:
    case GET -> Root / "api" / "v1" / "admin" / "secret" => Ok(Json.obj("data" -> "classified".asJson))
  ...
  test("request without cookie skips protected routes (404)") {
    val protected_ = AuthMiddleware(mkAuthService)(innerRoutes).orNotFound
    val req = Request[IO](Method.GET, uri"/api/v1/admin/secret")
  ...
  test("request with invalid token skips protected routes (404)") {
    val protected_ = AuthMiddleware(mkAuthService)(innerRoutes).orNotFound
    val req = Request[IO](Method.GET, uri"/api/v1/admin/secret").addCookie(sessionCookie("bad-token"))
    val resp = protected_.run(req).unsafeRunSync()
    assertEquals(resp.status, Status.NotFound)
  }
  ...
  test("request with valid token passes through to inner route") {
    val protected_ = AuthMiddleware(mkAuthService)(innerRoutes).orNotFound
    val req = Request[IO](Method.GET, uri"/api/v1/admin/secret").addCookie(sessionCookie(validToken))
    val resp = protected_.run(req).unsafeRunSync()
    assertEquals(resp.status, Status.Ok)
    val body = resp.as[Json].unsafeRunSync()
    assertEquals(body.hcursor.downField("data").as[String], Right("classified"))
  }

  test("valid token but unmatched path returns 404") {
    val protected_ = AuthMiddleware(mkAuthService)(innerRoutes).orNotFound
    val req = Request[IO](Method.GET, uri"/api/v1/admin/nonexistent").addCookie(sessionCookie(validToken))
    val resp = protected_.run(req).unsafeRunSync()
    assertEquals(resp.status, Status.NotFound)
  }
  ...
  test("protected routes compose with public routes") {
    val publicRoutes: HttpRoutes[IO] = HttpRoutes.of[IO]:
      case GET -> Root / "api" / "v1" / "public" => Ok(Json.obj("public" -> true.asJson))

    val combined = (AuthMiddleware(mkAuthService)(innerRoutes) <+> publicRoutes).orNotFound

    // Public route works without auth
    val pubReq = Request[IO](Method.GET, uri"/api/v1/public")
    ...
    val secReq = Request[IO](Method.GET, uri"/api/v1/admin/secret")
    val secResp = combined.run(secReq).unsafeRunSync()
    assertEquals(secResp.status, Status.NotFound)

    // Protected route works with auth
    val authReq = Request[IO](Method.GET, uri"/api/v1/admin/secret").addCookie(sessionCookie(validToken))
