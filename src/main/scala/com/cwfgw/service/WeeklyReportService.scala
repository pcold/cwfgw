...
/** Assembles the full weekly report data matching the PDF layout. Returns a grid: 13 team columns x 8 draft round rows,
  ...
  */
class WeeklyReportService(espnImportService: EspnImportService, xa: Transactor[IO])(using LoggerFactory[IO]):
  ...
  /** Orders tournaments by (startDate, name) to handle same-date multi-events like Week 8A / 8B.
    */
  private val tournamentOrd: Ordering[Tournament] = Ordering.by(t => (t.startDate, t.name))

  /** Finds the preview matching a tournament by ESPN ID. */
  private def matchPreview(previews: List[EspnLivePreview], tournament: Tournament): Option[EspnLivePreview] =
  ...
  private[service] def tBefore(a: Tournament, b: Tournament): Boolean = tournamentOrd.lt(a, b)

  /** True when `a` precedes or equals `b` in tournament order. */
  private[service] def tOnOrBefore(a: Tournament, b: Tournament): Boolean = tournamentOrd.lteq(a, b)
  ...
  def getReport(seasonId: UUID, tournamentId: UUID, live: Boolean = false): IO[Json] =
    val action =
      ...
      yield
        ...
        val throughTournaments = allTournaments.filter(t => tournament.forall(cur => tOnOrBefore(t, cur)))
        val throughScores = allScores.filter(s => throughTournaments.exists(_.id == s.tournamentId))
        ...
        val sideBetPerRound = rules.sideBetRounds.map { round =>
          ...
          val teamTotals = roundPicks.map { entry =>
            val total = throughScores.filter(s => s.teamId == entry.teamId && s.golferId == entry.golferId).map(_.points)
              .sum
          ...
          if teamTotals.isEmpty || teamTotals.values.forall(_ == BigDecimal(0)) then
            ...
            val payouts = teamTotals
              .map((tid, _) => if winners.contains(tid) then tid -> winnerCollects else tid -> -sideBetPerTeam)
            (round, teamTotals, payouts)
        }

        val sideBetResults: Map[UUID, BigDecimal] = sideBetPerRound.map(_._3)
          ...
          val rows = (1 to 8).toList.map: round =>
            roster.find(_.draftRound.contains(round)) match
              case None => Json.obj(
                  ...
                  "golfer_id" -> Json.Null,
                  "position" -> Json.Null,
                  "earnings" -> Json.fromBigDecimal(0),
                  "top_tens" -> 0.asJson,
              ...
              case Some(entry) =>
                ...
                val (seasonEarnings, seasonTopTens) = cumulativeByTeamGolfer
                  .getOrElse((team.id, entry.golferId), (BigDecimal(0), 0))
                Json.obj(
                  "round" -> round.asJson,
        ...
        val undraftedTopTens = results.filter(r => r.position.exists(_ <= 10) && !rosteredGolferIds.contains(r.golferId))
          .sortBy(_.position).map: r =>
            ...
            val stpStr = r.scoreToPar.map(s => if s == 0 then "E" else if s > 0 then s"+$s" else s.toString)
            Json.obj(
              "name" -> name.asJson,
        ...
        val sideBetDetail = sideBetPerRound.map: (round, cumEarnings, payouts) =>
          val teamEntries = teams.map: team =>
            ...
            val golferName = rosterEntry.flatMap(e => golferMap.get(e.golferId)).map(_.lastName.toUpperCase)
              .getOrElse("—")
        ...
        val report = Json.obj(
          ...
          "standings_order" ->
            teamColumns.sortBy(t => t.hcursor.downField("total_cash").as[BigDecimal].getOrElse(BigDecimal(0))).reverse
    ...
    action.transact(xa).flatMap { (baseReport, txRules, priorNonCompleted, selectedTournament) =>
      if !live then IO.pure(baseReport)
      else
        // 1. Overlay prior non-completed tournaments
        val withPrior = priorNonCompleted.sorted(tournamentOrd).foldLeft(IO.pure(baseReport)) { (accIO, t) =>
          accIO.flatMap { acc =>
            espnImportService.previewByDate(seasonId, t.startDate)
              .map(previews => matchPreview(previews, t).map(overlayPriorLivePreview(acc, _, txRules)).getOrElse(acc))
              .handleErrorWith(e => logger.warn(e)(s"Prior live overlay failed: ${t.name}").as(acc))
          }
        }
          ...
          if startDate.isEmpty then withPrior
            ...
            withPrior.flatMap { rpt =>
              espnImportService.previewByDate(seasonId, LocalDate.parse(startDate)).map { previews =>
                val matched = selectedTournament.flatMap(matchPreview(previews, _)).toList
                mergeLiveData(rpt, matched, txRules)
              }.handleErrorWith(e => logger.warn(e)(s"Live overlay failed for $tournamentId").as(rpt))
            }
        else withPrior
    }
  ...
  /** Season-wide report compiling data from all completed tournaments. When `live=true`, overlays ESPN data from
    ...
    */
  def getSeasonReport(seasonId: UUID, live: Boolean = false): IO[Json] =
    val action =
      for
        seasonOpt <- SeasonRepository.findById(seasonId)
        teams <- TeamRepository.findBySeason(seasonId)
      ...
      yield
        ...
        val sideBetPerTeam = rules.sideBetAmount
        val sideBetPerRound = buildSideBetPerRound(rules, allRosters, allScores, numTeams, sideBetPerTeam)
        val sideBetResults = aggregateSideBets(sideBetPerRound)

        val teamColumns = teams.map { team =>
          val roster = allRosters.filter(_.teamId == team.id).sortBy(_.draftRound)
          val rows = buildSeasonRows(roster, golferMap, cumulativeByTeamGolfer, team.id)
          val teamTopTens = topTensByTeam.getOrElse(team.id, BigDecimal(0))
        ...
        val rosteredGolferIds = allRosters.map(_.golferId).toSet
        val undraftedAgg = buildUndraftedAgg(allResults, completed, rosteredGolferIds, golferMap, rules)

        val sideBetDetail = buildSideBetDetail(sideBetPerRound, teams, allRosters, golferMap)

        val standingsOrder = teamColumns
          .sortBy(t => t.hcursor.downField("total_cash").as[BigDecimal].getOrElse(BigDecimal(0))).reverse.zipWithIndex
          .map((t, i) =>
            Json.obj(
              "rank" -> (i + 1).asJson,
              "team_name" -> t.hcursor.downField("team_name").as[String].getOrElse("").asJson,
    ...
    action.transact(xa).flatMap { (baseReport, rules, nonCompleted) =>
      if live && nonCompleted.nonEmpty then
        val sorted = nonCompleted.sorted(tournamentOrd)
        logger.info(s"Season report live overlay: " + s"${sorted.size} non-completed tournaments") >>
          sorted.foldLeft(IO.pure(baseReport)) { (accIO, tournament) =>
            accIO.flatMap { acc =>
              espnImportService.previewByDate(seasonId, tournament.startDate).map { previews =>
                val matched = matchPreview(previews, tournament).toList
                mergeLiveData(acc, matched, rules, additive = true)
              }.handleErrorWith(e => logger.warn(e)(s"Live overlay failed for " + s"${tournament.name}").as(acc))
            }
          }
      else if live then
        logger.info("Season report: live requested but " + "no non-completed tournaments found").as(baseReport)
  ...
  private def buildSeasonRows(
    ...
    teamId: UUID
  ): List[Json] = (1 to 8).toList.map { round =>
    roster.find(_.draftRound.contains(round)) match
      case None => Json.obj(
          "round" -> round.asJson,
          "golfer_name" -> Json.Null,
  ...
  private def buildSideBetDetail(
    ...
    allRosters: List[RosterEntry],
    golferMap: Map[UUID, Golfer]
  ): List[Json] = perRound.map { (round, cumEarnings, payouts) =>
    val teamEntries = teams.map { team =>
  ...
  private def buildUndraftedAgg(
    ...
    golferMap: Map[UUID, Golfer],
    rules: SeasonRules
  ): List[Json] = allResults.filter(r => r.position.exists(_ <= 10) && !rosteredGolferIds.contains(r.golferId))
    .groupBy(_.golferId).toList.map { (golferId, results) =>
      ...
      Json.obj("name" -> name.asJson, "position" -> Json.Null, "payout" -> totalPayout.asJson)
    }.sortBy(j => j.hcursor.downField("payout").as[BigDecimal].getOrElse(BigDecimal(0))).reverse
    ...
    val action =
      ...
      yield
        ...
        val results = scores.map: (tournamentName, position, earnings, basePayout) =>
          Json.obj("tournament" -> tournamentName.asJson, "position" -> position.asJson, "earnings" -> earnings.asJson)
        Json.obj(
  ...
  /** Rankings with historical cumulative totals per team through an optional tournament cutoff.
    */
  def getRankings(seasonId: UUID, live: Boolean = false, throughTournamentId: Option[UUID] = None): IO[Json] =
    val action =
      for
        seasonOpt <- SeasonRepository.findById(seasonId)
        teams <- TeamRepository.findBySeason(seasonId)
      ...
      yield
        ...
        val sideBetResults = computeSideBets(sideBetCumulative, numTeams, sideBetPerTeam)

        val history = buildCumulativeHistory(sorted, allScores, teams, numTeams)

        val weekLabels = sorted.map(t => t.metadata.hcursor.downField("week").as[String].getOrElse(""))
    ...
    action.transact(xa).flatMap: (baseRankings, liveCandidates, ctx) =>
      if live && liveCandidates.nonEmpty then
        liveCandidates.foldLeft(IO.pure((baseRankings, ctx))) { (accIO, tournament) =>
          accIO.flatMap((acc, accCtx) => overlayLiveRankings(seasonId, acc, tournament, accCtx))
        }.map(_._1)
  ...
  private def scopedSideBetTotal(
    ...
    if tournamentIds.isEmpty then BigDecimal(0).pure[ConnectionIO]
      ...
      val inClause = Fragments.in(fr"fs.tournament_id", cats.data.NonEmptyList.fromListUnsafe(tournamentIds.toList))
    ...
    espnImportService.previewByDate(seasonId, tournament.startDate).map { previews =>
      matchPreview(previews, tournament) match
        ...
        case Some(preview) =>
          ...
          val liveByGolfer: Map[(UUID, UUID), BigDecimal] = preview.teams
            .flatMap(t => t.golferScores.map(gs => (t.teamId, gs.golferId) -> gs.payout)).toMap

          // Update side bet cumulative with live earnings
          val updatedCumulative = ctx.sideBetCumulative.map { (round, teamTotals) =>
            val withLive = ctx.allRosters.filter(_.draftRound.contains(round)).foldLeft(teamTotals) { (acc, entry) =>
              val liveEarnings = liveByGolfer.getOrElse((entry.teamId, entry.golferId), BigDecimal(0))
              val adjusted = liveEarnings * entry.ownershipPct / 100
              if adjusted == BigDecimal(0) then acc
              else acc.updated(entry.teamId, acc.getOrElse(entry.teamId, BigDecimal(0)) + adjusted)
            }
            (round, withLive)
          }
          ...
          val updatedTeams = teams.map { t =>
            ...
            val sideBets = newSideBets.getOrElse(teamUuid, BigDecimal(0))
            ...
            t.mapObject(
              _.add("subtotal", newSubtotal.asJson).add("side_bets", sideBets.asJson).add("total_cash", newTotal.asJson)
                .add("series", (series :+ newTotal).asJson).add("live_weekly", liveWeeklyTotal.asJson)
            )
          }
          ...
          val sortedTeams = updatedTeams
            .sortBy(t => t.hcursor.downField("total_cash").as[BigDecimal].getOrElse(BigDecimal(0))).reverse
          val updatedRankings = baseRankings.mapObject(
            _.add("teams", sortedTeams.asJson).add("weeks", (weeks :+ weekLabel).asJson)
              .add("tournament_names", (names :+ (preview.espnName + " *")).asJson).add("live", Json.True)
          )
          (updatedRankings, ctx.copy(sideBetCumulative = updatedCumulative))
    }.handleError(_ => (baseRankings, ctx))

  /** Overlay a prior non-completed tournament's ESPN data onto a report. Adds the tournament's zero-sum totals to
    * `previous` and live golfer earnings to `season_earnings`/`season_top_tens`. Does NOT modify per-golfer `earnings`
    * (those belong to the selected tournament). Also updates side bet cumulative data.
    */
  private[service] def overlayPriorLivePreview(report: Json, preview: EspnLivePreview, rules: SeasonRules): Json =
    val teams = report.hcursor.downField("teams").as[List[Json]].getOrElse(Nil)
    val numTeams = teams.size
    if numTeams == 0 then return report

    val totalPot = preview.teams.map(_.topTenEarnings).sum
    val zeroSumByTeam: Map[String, BigDecimal] = preview.teams
      .map(t => t.teamId.toString -> (t.topTenEarnings * numTeams - totalPot)).toMap

    val golferPayouts: Map[(String, String), (BigDecimal, Int)] = preview.teams
      .flatMap(t => t.golferScores.map(gs => (t.teamId.toString, gs.golferId.toString) -> (gs.payout, gs.position)))
      .toMap
    ...
    val updatedTeams = teams.map { teamJson =>
      ...
      val weeklyTotal = teamJson.hcursor.downField("weekly_total").as[BigDecimal].getOrElse(BigDecimal(0))
      ...
      val topTenMoney = teamJson.hcursor.downField("top_ten_money").as[BigDecimal].getOrElse(BigDecimal(0))

      val priorWeekly = zeroSumByTeam.getOrElse(teamId, BigDecimal(0))
      ...
      val updatedRows = rows.map { row =>
        ...
        golferPayouts.get((teamId, golferId)) match
          case Some((payout, _)) =>
            addedTopTens += 1
            addedMoney += payout
            val se = row.hcursor.downField("season_earnings").as[BigDecimal].getOrElse(BigDecimal(0))
            val st = row.hcursor.downField("season_top_tens").as[Int].getOrElse(0)
            row.mapObject(_.add("season_earnings", (se + payout).asJson).add("season_top_tens", (st + 1).asJson))
          case None => row
      }

      teamJson.mapObject(
        _.add("previous", newPrevious.asJson).add("subtotal", newSubtotal.asJson)
          .add("total_cash", (newSubtotal + sideBets).asJson).add("rows", updatedRows.asJson)
          .add("top_ten_count", (topTenCount + addedTopTens).asJson)
          .add("top_ten_money", (topTenMoney + addedMoney).asJson)
      )
    }
    ...
    val updatedSideBetDetail = baseSideBetDetail.map { rdJson =>
      ...
      val updatedRdTeams = rdTeams.map { entry =>
        ...
        val gidOpt = updatedTeams.find(t => t.hcursor.downField("team_id").as[String].getOrElse("") == teamId)
          .flatMap { t =>
            t.hcursor.downField("rows").as[List[Json]].getOrElse(Nil)
              .find(r => r.hcursor.downField("round").as[Int].getOrElse(0) == round)
              .flatMap(r => r.hcursor.downField("golfer_id").as[String].toOption)
          }
        val liveEarnings = gidOpt.flatMap(gid => golferPayouts.get((teamId, gid))).map(_._1).getOrElse(BigDecimal(0))
        val ownershipPct = updatedTeams.find(t => t.hcursor.downField("team_id").as[String].getOrElse("") == teamId)
          .flatMap { t =>
            t.hcursor.downField("rows").as[List[Json]].getOrElse(Nil)
              .find(r => r.hcursor.downField("round").as[Int].getOrElse(0) == round)
        ...
        entry.mapObject(_.add("cumulative_earnings", (cumEarnings + adjusted).asJson))
      }
      // Recompute round payouts
      val teamEarnings = updatedRdTeams.map(e =>
        e.hcursor.downField("team_id").as[String].getOrElse("") -> e.hcursor.downField("cumulative_earnings")
          .as[BigDecimal].getOrElse(BigDecimal(0))
      ).toMap
      val allZero = teamEarnings.values.forall(_ == BigDecimal(0))
      val finalRdTeams =
        if allZero then updatedRdTeams.map(_.mapObject(_.add("payout", BigDecimal(0).asJson)))
        else
    ...
    val sideBetTotals: Map[String, BigDecimal] = updatedSideBetDetail.flatMap { rdJson =>
      rdJson.hcursor.downField("teams").as[List[Json]].getOrElse(Nil).map { e =>
        e.hcursor.downField("team_id").as[String].getOrElse("") -> e.hcursor.downField("payout")
          .as[BigDecimal].getOrElse(BigDecimal(0))
      }
    }.groupBy(_._1).view.mapValues(_.map(_._2).sum).toMap
    ...
    val finalTeams = updatedTeams.map { t =>
      val teamId = t.hcursor.downField("team_id").as[String].getOrElse("")
      val subtotal = t.hcursor.downField("subtotal").as[BigDecimal].getOrElse(BigDecimal(0))
      val newSideBets = sideBetTotals.getOrElse(teamId, BigDecimal(0))
      t.mapObject(_.add("side_bets", newSideBets.asJson).add("total_cash", (subtotal + newSideBets).asJson))
    }

    val standingsOrder = finalTeams.sortBy(t => t.hcursor.downField("total_cash").as[BigDecimal].getOrElse(BigDecimal(0)))
    ...
    report.mapObject(
      _.add("teams", finalTeams.asJson).add("side_bet_detail", updatedSideBetDetail.asJson)
        .add("standings_order", standingsOrder.asJson).add("live", Json.True)
    )

  /** Overlay ESPN live preview data onto the base report. Replaces per-golfer earnings with live projected payouts and
    * recomputes weekly totals, subtotals, and total cash.
    */
  private[service] def mergeLiveData(
    ...
    preview match
      ...
      case Some(liveData) =>
        ...
        val livePayout: Map[(String, String), (BigDecimal, Int, Option[Int])] = liveData.teams.flatMap: team =>
        ...
        val updatedTeams = teams.map: teamJson =>
          ...
          val updatedRows = rows.map: row =>
            ...
            livePayout.get((teamId, golferId)) match
              case Some((payout, pos, stp)) =>
                ...
                val newEarnings = if additive then baseEarnings + payout else payout
                val newTopTens = if additive then baseTopTens + 1 else 1
              ...
              case None =>
                ...
                else row.deepMerge(Json.obj("earnings" -> BigDecimal(0).asJson, "top_tens" -> 0.asJson))

          val weeklyTopTens = updatedRows.foldLeft(BigDecimal(0))((acc, r) =>
            acc + r.hcursor.downField("earnings").as[BigDecimal].getOrElse(BigDecimal(0))
          )
          val liveTopTenCount =
            if additive then updatedRows.map(r => r.hcursor.downField("top_tens").as[Int].getOrElse(0)).sum
            else topTenCount + updatedRows.count(r => r.hcursor.downField("top_tens").as[Int].getOrElse(0) > 0)
          ...
          teamJson.deepMerge(Json.obj("rows" -> updatedRows.asJson, "top_tens" -> weeklyTopTens.asJson))
            ...
            .mapObject(
              _.add("_weekly_top_tens", weeklyTopTens.asJson).add("_previous", previous.asJson)
                .add("_side_bets", sideBets.asJson).add("_top_ten_count", liveTopTenCount.asJson)
            )
        ...
        val liveSideBetDetail = baseSideBetDetail.map: rdJson =>
          ...
          val updatedRdTeams = rdTeams.map: entry =>
            ...
            val golferIdOpt = updatedTeams.find(t => t.hcursor.downField("team_id").as[String].getOrElse("") == teamId)
              .flatMap: t =>
                ...
                rows.find(r => r.hcursor.downField("round").as[Int].getOrElse(0) == round)
                  .flatMap(r => r.hcursor.downField("golfer_id").as[String].toOption)
            val liveEarnings = golferIdOpt.flatMap(gid => livePayout.get((teamId, gid))).map(_._1)
        ...
        val liveSideBetTotals: Map[String, BigDecimal] = liveSideBetDetail.flatMap: rdJson =>
          rdJson.hcursor.downField("teams").as[List[Json]].getOrElse(Nil).map: e =>
            e.hcursor.downField("team_id").as[String].getOrElse("") -> e.hcursor.downField("payout")
              .as[BigDecimal].getOrElse(BigDecimal(0))
        .groupBy(_._1).view.mapValues(_.map(_._2).sum).toMap
        ...
        val finalTeams = updatedTeams.map: t =>
          ...
          val sideBets =
            if liveSideBetTotals.nonEmpty then liveSideBetTotals.getOrElse(teamId, BigDecimal(0)) else originalSideBets
          val topTenCount = t.hcursor.downField("_top_ten_count").as[Int].getOrElse(0)
          val weeklyTotal = weeklyTopTens * numTeams - totalPot
          val subtotal = previous + weeklyTotal
          val totalCash = subtotal + sideBets
          t.mapObject(
            _.add("weekly_total", weeklyTotal.asJson).add("subtotal", subtotal.asJson).add("side_bets", sideBets.asJson)
              .add("total_cash", totalCash.asJson).add("top_ten_count", topTenCount.asJson).remove("_weekly_top_tens")
              .remove("_previous").remove("_side_bets").remove("_top_ten_count")
          )
        ...
        report.mapObject(
          _.add("teams", finalTeams.asJson).add("side_bet_detail", liveSideBetDetail.asJson)
            .add("standings_order", standingsOrder.asJson).add("live", Json.True)
        )
