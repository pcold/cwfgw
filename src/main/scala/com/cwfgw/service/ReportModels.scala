...
case class UndraftedGolfer(
    name: String,
    position: Option[Int],
    payout: BigDecimal,
    scoreToPar: Option[String] = None
) derives ConfiguredCodec

/** A team's entry in a side bet detail round. */
case class ReportSideBetTeamEntry(
    teamId: UUID,
    golferName: String,
...
case class ReportSideBetRound(
    round: Int,
    teams: List[ReportSideBetTeamEntry]
) derives ConfiguredCodec

/** An entry in the standings order. */
case class StandingsEntry(
    rank: Int,
    teamName: String,
    totalCash: BigDecimal
) derives ConfiguredCodec
