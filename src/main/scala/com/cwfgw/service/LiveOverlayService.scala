...
/** Merges ESPN live leaderboard data onto base reports and rankings. All pure transformation methods are
  * package-private for testability.
  */
class LiveOverlayService(espnImportService: EspnImportService)(using LoggerFactory[IO]):
  ...
  /** Overlay live ESPN data onto a single-tournament report. Handles both prior non-completed tournaments and the
    * selected tournament itself.
    */
  def overlayReport(
    seasonId: UUID,
    ...
    val withPrior = priorNonCompleted.sorted(tournamentOrd).foldLeft(IO.pure(baseReport)) { (accIO, t) =>
      accIO.flatMap { acc =>
        espnImportService.previewByDate(seasonId, t.startDate).map { previews =>
          matchPreview(previews, t).map(overlayPriorLivePreview(acc, _, rules)).getOrElse(acc)
      ...
      logger.info(s"Season report live overlay: ${sorted.size} non-completed tournaments") >>
        sorted.foldLeft(IO.pure(baseReport)) { (accIO, tournament) =>
          accIO.flatMap { acc =>
            ...
            }.handleErrorWith { e => logger.warn(e)(s"Live overlay failed for ${tournament.name}").as(acc) }
          }
        }
    else logger.info("Season report: live requested but no non-completed tournaments found").as(baseReport)
  ...
  def overlayRankings(
    ...
    ctx: RankingsContext
  ): IO[Rankings] = liveCandidates.foldLeft(IO.pure((baseRankings, ctx))) { (accIO, tournament) =>
    ...
    espnImportService.previewByDate(seasonId, tournament.startDate).map { previews =>
      matchPreview(previews, tournament) match
        ...
        case Some(preview) =>
          ...
          val liveWeekly = preview.teams.map { t => t.teamId -> (t.topTenEarnings * numTeams - totalPot) }.toMap
          val weekLabel = tournament.week.getOrElse("")

          val liveByGolfer = preview.teams.flatMap { t => t.golferScores.map(gs => (t.teamId, gs.golferId) -> gs.payout) }
            .toMap
  ...
  /** Overlay a prior non-completed tournament's ESPN data onto a report. Adds the tournament's zero-sum totals to
    * `previous` and live golfer earnings to `seasonEarnings`/`seasonTopTens`. Does NOT modify per-golfer `earnings`
  ...
  private[service] def overlayPriorLivePreview(
    ...
    if numTeams == 0 then report
      ...
      val totalPot = preview.teams.map(_.topTenEarnings).sum
      val zeroSumByTeam: Map[UUID, BigDecimal] = preview.teams
        .map(t => t.teamId -> (t.topTenEarnings * numTeams - totalPot)).toMap

      val golferPayouts: Map[(UUID, UUID), (BigDecimal, Int)] = preview.teams.flatMap { t =>
      ...
      val updatedTeams = report.teams.map { team =>
        ...
        val (updatedRows, addedTopTens, addedMoney) = team.rows
          .foldLeft((List.empty[ReportRow], 0, BigDecimal(0))) { case ((acc, topTens, money), row) =>
            row.golferId.flatMap(gid => golferPayouts.get((team.teamId, gid))) match
              case Some((payout, _)) =>
                val updated = row
                  .copy(seasonEarnings = row.seasonEarnings + payout, seasonTopTens = row.seasonTopTens + 1)
                (acc :+ updated, topTens + 1, money + payout)
              case None => (acc :+ row, topTens, money)
          }
  ...
  ): WeeklyReport = previews.headOption match
    case None => report
    case Some(liveData) =>
      val livePayout: Map[(UUID, UUID), (BigDecimal, Int, Option[Int])] = liveData.teams.flatMap { team =>
        team.golferScores.map { gs => (team.teamId, gs.golferId) -> (gs.payout, gs.position, gs.scoreToPar) }
      }.toMap
      ...
      val phase1Teams = report.teams.map { team =>
        val updatedRows = team.rows.map { row =>
          row.golferId.flatMap(gid => livePayout.get((team.teamId, gid))) match
            ...
            case None => if additive then row else row.copy(earnings = BigDecimal(0), topTens = 0)
        }

        val weeklyTopTens = updatedRows.map(_.earnings).sum
        val liveTopTenCount =
          if additive then updatedRows.map(_.topTens).sum else team.topTenCount + updatedRows.count(_.topTens > 0)
      ...
      val finalTeams = phase1Teams.map { (team, weeklyTopTens, previous, origSideBets) =>
        val sideBets =
          if liveSideBetTotals.nonEmpty then liveSideBetTotals.getOrElse(team.teamId, BigDecimal(0)) else origSideBets
        val weeklyTotal = weeklyTopTens * numTeams - totalPot
        val subtotal = previous + weeklyTotal
        val totalCash = subtotal + sideBets
        team.copy(weeklyTotal = weeklyTotal, subtotal = subtotal, sideBets = sideBets, totalCash = totalCash)
      }
  ...
  /** Update side bet detail entries with live golfer earnings. The earningsLookup function resolves a (teamId,
    * golferId) pair to the live earnings amount.
    */
  private def updateSideBetDetailWith(
    ...
    numTeams: Int,
    sideBetPerTeam: BigDecimal
  ): List[ReportSideBetRound] = sideBetDetail.map { rd =>
    val updatedEntries = rd.teams.map { entry =>
      val gidOpt = teams.find(_.teamId == entry.teamId).flatMap(_.rows.find(_.round == rd.round).flatMap(_.golferId))
      val liveEarnings = gidOpt.map(gid => earningsLookup(entry.teamId, gid)).getOrElse(BigDecimal(0))
      val ownershipPct = teams.find(_.teamId == entry.teamId)
        .flatMap(_.rows.find(_.round == rd.round).map(_.ownershipPct)).getOrElse(BigDecimal(100))
  ...
  private def aggregateSideBetTotals(sideBetDetail: List[ReportSideBetRound]): Map[UUID, BigDecimal] = sideBetDetail
    .flatMap(_.teams.map(e => e.teamId -> e.payout)).groupBy(_._1).view.mapValues(_.map(_._2).sum).toMap
