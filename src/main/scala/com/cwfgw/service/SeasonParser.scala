...
/** Parses a season schedule text file into tournament entries.
  ...
  * - Trailing "Nx" (e.g. "2x", "1.5x") sets the payout
  *   multiplier for that tournament. No suffix = 1x.
...
object SeasonParser:
  ...
  def parse(
      text: String,
      seasonYear: Int
  ): Either[String, List[ParsedTournament]] =
    val lines = text.trim.linesIterator
      .filter(_.trim.nonEmpty).toList
    val results = lines.zipWithIndex.map { (line, idx) =>
      parseLine(line.trim, seasonYear)
        .left.map(err => s"Line ${idx + 1}: $err")
    }
  ...
  private def parseLine(
      line: String,
      year: Int
  ): Either[String, ParsedTournament] =
    val tokens = line.split("\\s+").toList
    tokens match
      case week :: eventStr :: rest
          if eventStr.toIntOption.isDefined =>
        val eventNum = eventStr.toInt
        parseDateAndName(rest, year).map {
          (start, end, name, mult, isSig, notes) =>
            ParsedTournament(
              week, eventNum, start, end,
              name, mult, isSig, notes
            )
        }
      case _ =>
        Left(
          s"Could not parse: '$line' — " +
            "expected: WEEK EVENT_NUM DATE_RANGE NAME"
        )
  ...
  private def parseDateAndName(
      ...
      BigDecimal, Boolean, Option[String]
  )] =
    tokens match
      case monthStr :: dateRange :: rest =>
        parseMonth(monthStr) match
          case None =>
            Left(s"Unknown month: '$monthStr'")
          case Some(startMonth) =>
            parseDateRange(dateRange, startMonth, year)
              .flatMap { (startDate, endDate) =>
                ...
                val (nameWithoutMult, multiplier) =
                  extractMultiplier(rawName)

                // Also support legacy "Double $$" for
                // backward compatibility
                val (cleanName, finalMult) =
                  if nameWithoutMult.contains("Double $$") then
                    val n = nameWithoutMult
                      .replace("Double $$", "").trim
                    (n, BigDecimal(2))
                  else
                    (nameWithoutMult, multiplier)

                val name = cleanName
                  .replace("***", "")
                  .replaceAll(
                    "\\s*(2 event week|BOTH count)\\s*",
                    ""
                  )
                  .trim
                ...
                val notes = List(
                  ...
                  if rawName.contains("2 event week") then
                    Some("2 event week")
                  else None,
                  if rawName.contains("BOTH count") then
                    Some("BOTH count")
                  else None
                ).flatten match
                  case Nil => None
                ...
                Right((
                  ...
                  finalMult, isSignature, notes
                ))
              }
      case _ =>
        Left("Missing date range")

  /** Extract a trailing multiplier like "2x" or "1.5x"
    * from the raw name string. Returns the name without
    * the multiplier token and the parsed value. */
  private def extractMultiplier(
      raw: String
  ): (String, BigDecimal) =
    val tokens = raw.split("\\s+").toList
    tokens.lastOption match
      case Some(last) =>
        multiplierPattern.findFirstMatchIn(last) match
          case Some(m) =>
  ...
  /** Parse a date range like "15-18" (same month),
    * "29-Feb1" (cross-month), "5-8" */
  private def parseDateRange(
      range: String,
      startMonth: Month,
      year: Int
  ): Either[String, (LocalDate, LocalDate)] =
    range.split("-", 2) match
      case Array(startStr, endStr) =>
        startStr.toIntOption match
          case None =>
            Left(s"Invalid start day: '$startStr'")
          case Some(startDay) =>
            endStr.toIntOption match
              case Some(endDay) =>
                ...
                val end = LocalDate.of(
                  year, endMonth, endDay
                )
                Right((start, end))
              case None =>
                val monthEnd = months.keys
                  .find(m =>
                    endStr.toLowerCase.startsWith(m)
                  )
                monthEnd match
                  case Some(mStr) =>
                    val dayStr =
                      endStr.substring(mStr.length)
                    dayStr.toIntOption match
                      case Some(endDay) =>
                        ...
                        val end = LocalDate.of(
                          year, months(mStr), endDay
                        )
                        Right((start, end))
                      case None =>
                        Left(
                          s"Invalid end date: '$endStr'"
                        )
                  case None =>
                    Left(s"Invalid end date: '$endStr'")
      case _ =>
        Left(s"Invalid date range: '$range'")
