...
class AdminService(espnClient: EspnClient, xa: Transactor[IO])(using LoggerFactory[IO]):
  ...
  def uploadSeason(leagueId: UUID, seasonYear: Int, scheduleText: String): IO[SeasonUploadResult] =
    for
      ...
      espnCalendar <- espnClient.fetchCalendar.handleErrorWith: e =>
        logger.warn(s"ESPN calendar fetch failed: ${e.getMessage}") >>
          IO.pure(Nil)
  ...
  /** Step 1: Parse roster text and match each player against ESPN athletes.
    * Returns match results with suggestions for ambiguous/unmatched names. */
  def previewRoster(rosterText: String): IO[RosterPreviewResult] =
    for
      parsed <- IO.fromEither(
        RosterParser.parse(rosterText).left.map(e => new RuntimeException(s"Parse error:\n$e"))
      )
      ...
      val teamPreviews = parsed.map: team =>
        val pickPreviews = team.picks.map: pick =>
          matchEspnPlayer(pick.playerName, espnAthletes) match
            case MatchResult.Exact(athlete) =>
              RosterPickPreview(pick.round, pick.playerName, pick.ownershipPct,
                "exact", Some(athlete.espnId), Some(athlete.name), Nil)
            case MatchResult.Ambiguous(candidates) =>
              RosterPickPreview(pick.round, pick.playerName, pick.ownershipPct,
                "ambiguous", None, None, candidates.map(a => EspnSuggestion(a.espnId, a.name)))
            case MatchResult.NoMatch =>
              RosterPickPreview(pick.round, pick.playerName, pick.ownershipPct,
                "no_match", None, None, Nil)
        ...
        for
          ...
          pickResults <- team.picks.traverse: pick =>
            for
              ...
              _ <- TeamRepository.addToRoster(created.id, AddToRoster(
                golferId = golfer.id,
  ...
  private val playerAliases: Map[String, String] = Map(
    "AN" -> "Byeong-Hun An",
  ...
  private def matchEspnPlayer(name: String, athletes: List[EspnAthlete]): MatchResult =
    ...
    playerAliases.get(normalized).orElse(playerAliases.get(normalized.split("[\\s,.]+").last)) match
      case Some(aliasName) =>
        athletes.find(a => stripDiacritics(a.name).equalsIgnoreCase(aliasName)) match
    ...
    val (lastName, firstHint) = if normalized.contains(",") then
      ...
      if tokens.length >= 2 then
        ...
        (normalized.replaceAll("\\.", "").trim, None)
    ...
    narrowByFirstHint(exactMatches, firstHint) match
      ...
      case _ =>
        ...
        val fuzzyMatches = athletes
          .map(a => (a, editDistance(stripDiacritics(espnLastName(a).toUpperCase), lastName)))
          .filter(_._2 <= maxDist)
          .sortBy(_._2)
          .map(_._1)
        ...
        narrowByFirstHint(fuzzyMatches, firstHint) match
          ...
          case _ =>
            ...
            val fullNameFuzzy = athletes
              .map(a => (a, editDistance(stripDiacritics(a.name.toUpperCase), normalized)))
              .filter(_._2 <= 3)
              .sortBy(_._2)
  ...
  private def findOrCreateGolfer(
      rosterName: String, espnId: Option[String], espnName: Option[String]
  ): ConnectionIO[Golfer] =
    espnId match
      case Some(eid) =>
  ...
  def previewEspnCalendar: IO[List[EspnCalendarEntry]] =
    ...
    calendar.find(e => normalize(e.label) == normalized)
      ...
      .orElse:
        ...
        if words.isEmpty then None
          ...
          calendar
            .map(e => (e, wordOverlap(words, normalize(e.label))))
            .filter(_._2 > 0.5)
            .sortBy(-_._2)
            .headOption
            .map(_._1)
...
case class RosterTeamPreview(
    teamNumber: Int,
    teamName: String,
    picks: List[RosterPickPreview]
)
...
case class ConfirmedTeam(
    teamNumber: Int,
    teamName: String,
    picks: List[ConfirmedPick]
)
...
case class RosterUploadResult(
    teamsCreated: Int,
    golfersCreated: Int,
    teams: List[TeamUploadResult]
)

case class TeamUploadResult(
    teamId: UUID,
    teamNumber: Int,
    teamName: String,
    picks: List[RosterPickResult]
)

case class RosterPickResult(
    round: Int,
    golferName: String,
    golferId: UUID,
    ownershipPct: Int,
