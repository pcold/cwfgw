...
class DraftService(xa: Transactor[IO]):
  ...
  def get(seasonId: UUID): IO[Option[Draft]] = DraftRepository.findBySeason(seasonId).transact(xa)

  def create(seasonId: UUID, req: CreateDraft): IO[Draft] = DraftRepository.create(seasonId, req).transact(xa)
    ...
    val action =
      for
        ...
        result <- draftOpt match
          ...
          case Some(draft) if draft.status != "pending" => FC.pure(Left(s"Draft is already ${draft.status}"))
          case Some(draft) => DraftRepository.updateStatus(draft.id, "in_progress").map {
              case Some(d) => Right(d)
  ...
  def makePick(seasonId: UUID, req: MakePick): IO[Either[String, DraftPick]] =
    val action =
      for
        ...
        result <- draftOpt match
          case None => FC.pure(Left("No draft found for this season"))
          case Some(draft) if draft.status != "in_progress" => FC.pure(Left("Draft is not in progress"))
          case Some(draft) =>
            for
              ...
              result <- nextPick match
                ...
                case Some(pick) if pick.teamId != req.teamId => FC.pure(Left(s"It is not this team's turn to pick"))
                case Some(pick) => DraftRepository.makePick(draft.id, pick.pickNum, req.golferId).map {
                    case Some(p) => Right(p)
  ...
  /** Generate snake draft pick order: odd rounds go in team order, even rounds reverse. Returns (draftId, teamId,
    * roundNum, pickNum).
    */
  private[service] def snakeDraftOrder(teamIds: List[UUID], rounds: Int, draftId: UUID): List[(UUID, UUID, Int, Int)] =
    for
