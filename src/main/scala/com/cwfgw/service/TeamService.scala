...
class TeamService(xa: Transactor[IO]):
  ...
  def listBySeason(seasonId: UUID): IO[List[Team]] = TeamRepository.findBySeason(seasonId).transact(xa)

  def get(id: UUID): IO[Option[Team]] = TeamRepository.findById(id).transact(xa)

  def create(seasonId: UUID, req: CreateTeam): IO[Team] = TeamRepository.create(seasonId, req).transact(xa)

  def update(id: UUID, req: UpdateTeam): IO[Option[Team]] = TeamRepository.update(id, req).transact(xa)

  def getRoster(teamId: UUID): IO[List[RosterEntry]] = TeamRepository.getRoster(teamId).transact(xa)

  def addToRoster(teamId: UUID, req: AddToRoster): IO[RosterEntry] = TeamRepository.addToRoster(teamId, req).transact(xa)

  def dropFromRoster(teamId: UUID, golferId: UUID): IO[Boolean] = TeamRepository.dropFromRoster(teamId, golferId)
    .transact(xa)
