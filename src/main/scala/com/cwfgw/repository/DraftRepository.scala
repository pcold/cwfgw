...
object DraftRepository:
  ...
  def getPicks(draftId: UUID): ConnectionIO[List[DraftPick]] =
    (fr"SELECT" ++ selectPickCols ++ fr"FROM draft_picks WHERE draft_id = $draftId ORDER BY pick_num")
      .query[DraftPick].to[List]
  ...
  def createPick(draftId: UUID, teamId: UUID, roundNum: Int, pickNum: Int): ConnectionIO[DraftPick] =
    sql"""INSERT INTO draft_picks (draft_id, team_id, round_num, pick_num)
          ...
          RETURNING $selectPickCols"""
      .query[DraftPick].unique

  def makePick(draftId: UUID, pickNum: Int, golferId: UUID): ConnectionIO[Option[DraftPick]] =
    (fr"UPDATE draft_picks SET golfer_id = $golferId, picked_at = now() WHERE draft_id = $draftId AND pick_num = $pickNum AND golfer_id IS NULL RETURNING" ++ selectPickCols)
      .query[DraftPick].option
