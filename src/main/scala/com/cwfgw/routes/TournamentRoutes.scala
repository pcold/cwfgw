...
object TournamentRoutes:
  ...
  private object SeasonIdParam extends OptionalQueryParamDecoderMatcher[UUID]("season_id")
  ...
  def routes(service: TournamentService): HttpRoutes[IO] = HttpRoutes.of[IO]:
    case GET -> Root / "api" / "v1" / "tournaments" :? SeasonIdParam(seasonId) +& StatusParam(status) => service
        .list(seasonId, status).flatMap(Ok(_))

    case GET -> Root / "api" / "v1" / "tournaments" / UUIDVar(id) => service.get(id).flatMap:
        case Some(t) => Ok(t)
        case None => NotFound()

    case req @ POST -> Root / "api" / "v1" / "tournaments" => req.as[CreateTournament].flatMap: body =>
        service.create(body).flatMap(Created(_))

    case req @ PUT -> Root / "api" / "v1" / "tournaments" / UUIDVar(id) => req.as[UpdateTournament].flatMap: body =>
        service.update(id, body).flatMap:
          case Some(t) => Ok(t)
          case None => NotFound()

    case GET -> Root / "api" / "v1" / "tournaments" / UUIDVar(id) / "results" => service.getResults(id).flatMap(Ok(_))

    case req @ POST -> Root / "api" / "v1" / "tournaments" / UUIDVar(id) / "results" => req
        .as[List[CreateTournamentResult]].flatMap: body =>
