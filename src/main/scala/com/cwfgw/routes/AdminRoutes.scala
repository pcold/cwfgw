...
object AdminRoutes:
  ...
  def routes(service: AdminService)(using LoggerFactory[IO]): HttpRoutes[IO] = HttpRoutes.of[IO]:
    case req @ POST -> Root / "api" / "v1" / "admin" / "season" => req.as[SeasonUploadRequest].flatMap: body =>
        service.uploadSeason(body.seasonId, body.seasonYear, body.schedule).flatMap: result =>
          Ok(Json.obj(
                ...
                Json.obj(
                  ...
                  "picks" -> t.picks.map: p =>
                    Json.obj(
                      ...
                      "suggestions" ->
                        p.suggestions.map(s => Json.obj("espn_id" -> s.espnId.asJson, "name" -> s.name.asJson)).asJson
                    )
                  .asJson
                )
              .asJson
