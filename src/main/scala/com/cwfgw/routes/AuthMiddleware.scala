...
object AuthMiddleware:
  ...
  /** Middleware that gates access to inner routes behind a
    * valid session cookie. Requests without a valid session
    * are skipped (OptionT.none), allowing public routes
    * composed after these to still match.
    */
  def apply(
    ...
    Kleisli { req =>
      ...
      tokenOpt match
        case Some(token) =>
          OptionT(authService.validate(token).flatMap {
