...
/** ESPN PGA Tour scoreboard API client. Uses Java's built-in HttpClient which respects the system trust store, avoiding
  * issues with corporate SSL proxies (e.g. Zscaler).
  */
class EspnClient(httpClient: JHttpClient)(using LoggerFactory[IO]):
  ...
  def fetchScoreboard(date: LocalDate): IO[Json] =
    ...
    for
      ...
      body = response.body()
      _ <- IO.raiseWhen(response.statusCode() != 200)(new RuntimeException(s"ESPN returned ${response
          .statusCode()}: ${body.take(200)}"))
  ...
  private def parseEvent(event: Json): Either[String, EspnTournament] =
    for
      ...
      completed = event.hcursor.downField("status").downField("type").downField("completed").as[Boolean].getOrElse(false)
      competitorsJson <- event.hcursor.downField("competitions").downArray.downField("competitors").as[List[Json]].left
        .map(_.message)
      competitors = competitorsJson.zipWithIndex.flatMap((j, i) => parseCompetitor(j, i))
  ...
  /** Parse a single competitor from ESPN JSON. Falls back to list index for `order` when ESPN omits it.
    */
  ...
  /** Assign tied positions based on score. ESPN's `order` is sequential; players with the same score share a position
    * (e.g., T4).
    */
  ...
  def fetchCalendar: IO[List[EspnCalendarEntry]] =
    ...
    for
      ...
      entries = json.hcursor.downField("leagues").downArray.downField("calendar").as[List[Json]].toOption.getOrElse(Nil)
        .flatMap: j =>
          for
  ...
  /** Fetch athletes from PGA, LIV, and DP World Tour scoreboards. Pulls recent tournaments from each tour to build a
    * comprehensive player pool.
    */
    ...
    for
      ...
      currentAthletes <- tourUrls.traverse: url =>
        fetchAthletesFromUrl(url).handleErrorWith: e =>
          logger.warn(s"Failed to fetch from $url: ${e.getMessage}") >> IO.pure(Nil)

      // Also fetch from recent PGA tournament dates for broader coverage
      calendar <- fetchCalendar
      recentDates = calendar.flatMap(e => scala.util.Try(LocalDate.parse(e.startDate.take(10))).toOption)
        .filter(_.isBefore(LocalDate.now().plusDays(1))).sorted.reverse.take(6)
      ...
      historicalAthletes <- recentDates.traverse: date =>
        fetchScoreboardAthletes(date).handleErrorWith: e =>
          logger.warn(s"Failed to fetch athletes for $date: ${e.getMessage}") >> IO.pure(Nil)
  ...
  private def parseAthletesFromJson(json: Json): List[EspnAthlete] = json.hcursor.downField("events").as[List[Json]]
    .toOption.getOrElse(Nil).flatMap: event =>
      event.hcursor.downField("competitions").downArray.downField("competitors").as[List[Json]].toOption.getOrElse(Nil)
        .flatMap: c =>
          for
...
object EspnClient:
  def resource(using LoggerFactory[IO]): Resource[IO, EspnClient] = Resource.eval(IO.blocking {
